...
**Launching the server in OpenCode:**

```bash
scripts/start-server.sh --project-dir /path/to/project --open
```

The server backgrounds itself by default and writes connection details to `$STATE_DIR/server-info`. If your environment requires foreground execution, pass `--foreground` and run it with OpenCode's background shell support.
